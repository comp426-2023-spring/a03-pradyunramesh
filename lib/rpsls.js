export function rps(shot){
    //Function definition
    const shot_list = ['rock', 'paper', 'scissors']; //Create list of all possible shots for RPS
    ...
    if (shot == undefined){
        ...
        return {"player": random_shot}; //Return random shot selected
    }
    ...
    if (shot == random_shot){
        return {"player": shot, "opponent": random_shot, "result": "tie"}; //Return results for tie case
    }
...
export function rpsls(shot){
    const shot_list = ['rock', 'paper', 'scissors','lizard','spock']; //Create list of all 5 possible shots for RPSLS
    const random_index = Math.floor(Math.random()*3); //Generate random index
    const random_shot = shot_list[random_index]; //Generate random shot
    if (shot == undefined){
    ...
    if (shot == random_shot){
        return {"player": shot, "opponent": random_shot, "result": "tie"}; //Return result for tie case
    }
